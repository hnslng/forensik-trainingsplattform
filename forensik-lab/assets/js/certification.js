...
function _getCertKey() {
  ...
  return "certification-result-" + labId;
}

function saveCertificationResult(result) {
  try {
    localStorage.setItem(_getCertKey(), JSON.stringify(result));
  } catch (e) {
...
function getCertificationResult() {
  try {
    const result = localStorage.getItem(_getCertKey());
...
function validateCertificationRequirement() {
  ...
  const labId = (typeof App !== "undefined" && App.currentLab) ? App.currentLab : null;
  const chapterProgress = Progress.getCount(labId);
  const totalChapters = (typeof App !== "undefined" && App.navItems) ? App.navItems.length : 17;
  const minChaptersRequired = Math.ceil(totalChapters * 0.8);
  const chaptersCompleted = chapterProgress;
  
  if (chaptersCompleted < minChaptersRequired) {
    return {
      eligible: false,
      reason: `Du hast erst ${chaptersCompleted}/${totalChapters} Kapitel abgeschlossen. Mindestens ${minChaptersRequired} sind erforderlich für den Zertifizierungstest.`,
      canTakeTest: false
    };
  }
  ...
  return {
    eligible: true,
    reason: `Du hast ${chaptersCompleted}/${totalChapters} Kapitel abgeschlossen. Du kannst den Zertifizierungstest machen!`,
    canTakeTest: true
  };
}
