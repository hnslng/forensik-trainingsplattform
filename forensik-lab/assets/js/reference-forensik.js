var ReferenceForensik = {
	sections: [
		...
		},
		{
			...
			title: 'Glossar',
			icon: '📖',
			html: function() { return '<div class="s-heading">Forensik-Glossar</div>' +
				'<div class="s-table-wrap"><table class="s-table"><thead><tr><th>Begriff</th><th>Bedeutung</th></tr></thead><tbody>' +
				'<tr><td>Image</td><td>Bit-genauer Kopie eines Datenträgers (1:1 Abbild)</td></tr>' +
				'<tr><td>Hash</td><td>Kryptografische Prüfsumme zur Integritätsprüfung (z.B. SHA-256)</td></tr>' +
				'<tr><td>Write-Blocker</td><td>Gerät/Software das Schreibzugriffe auf den Originaldatenträger verhindert</td></tr>' +
				'<tr><td>Mount</td><td>Datenträger/Image in das Dateisystem einhängen (read-only in der Forensik)</td></tr>' +
				'<tr><td>Device</td><td>Blockgerät unter Linux (/dev/sda, /dev/nvme0n1)</td></tr>' +
				'<tr><td>Partition</td><td>Logischer Abschnitt eines Datenträgers (/dev/sda1)</td></tr>' +
				'<tr><td>Dateisystem</td><td>Struktur zur Organisation von Dateien (NTFS, ext4, FAT32)</td></tr>' +
				'<tr><td>MBR</td><td>Master Boot Record – ältere Partitionstabelle, max 4 primäre Partitionen, max 2 TiB</td></tr>' +
				'<tr><td>GPT</td><td>GUID Partition Table – modern, bis 128 Partitionen, Backup am Ende der Platte</td></tr>' +
				'<tr><td>Inode</td><td>Datenstruktur in ext-Dateisystemen: speichert Metadaten einer Datei</td></tr>' +
				'<tr><td>dd</td><td>Tool für bit-genau Kopieren (low-level)</td></tr>' +
				'<tr><td>dc3dd / dcfldd</td><td>Forensische dd-Varianten mit integriertem Hashing</td></tr>' +
				'<tr><td>E01</td><td>EnCase Image-Format mit Metadaten, Kompression und Hashing</td></tr>' +
				'<tr><td>AFF</td><td>Advanced Forensic Format – Open Source Alternative zu E01</td></tr>' +
				'<tr><td>Chain of Custody</td><td>Dokumentierte Beweiskette: wer hat was wann wie behandelt</td></tr>' +
				'<tr><td>File Carving</td><td>Wiederherstellung von Dateien anhand von Header/Footer-Signaturen</td></tr>' +
				'<tr><td>Timestamp</td><td>Zeitstempel einer Datei (MACB: Modified, Accessed, Changed, Birth)</td></tr>' +
				'<tr><td>Timeline</td><td>Zeitliche Darstellung forensischer Ereignisse</td></tr>' +
				'<tr><td>Slack Space</td><td>Unbenutzter Platz am Ende eines Clusters – kann alte Daten enthalten</td></tr>' +
				'<tr><td>Volatile Data</td><td>Flüchtige Daten: RAM-Inhalt, Netzwerkverbindungen, laufende Prozesse</td></tr>' +
				'</tbody></table></div>'; }
		}
	]
};
