var App = (function () {
  ...
  function loadChapter(chapterId) {
    ...
    // Try Chapters object first (static/embedded mode)
    ...
    // Fallback: try MarkdownLoader (server mode)
    if (typeof MarkdownLoader !== "undefined") {
      if (chapterId === "welcome") {
        ...
        bindContentEvents();
        injectTryButtons();
        var mc2 = document.getElementById("main-content"); if (mc2) mc2.scrollTop = 0;
        return;
      }
      MarkdownLoader.loadChapter(chapterId, function (mdHtml) {
        if (!mdHtml) {
          el.innerHTML = '<div class="fade-in">' + renderWelcome() + "</div>";
          chapterId = "welcome";
        } else {
          parseAndRenderSlides(el, mdHtml);
        }
        updateBreadcrumb(chapterId);
        highlightNav(chapterId);
        bindContentEvents();
        injectTryButtons();
        var mc3 = document.getElementById("main-content"); if (mc3) mc3.scrollTop = 0;
        setTimeout(updateScrollFade, 100);
      });
      return;
    }

    // Last resort: show welcome
    el.innerHTML = '<div class="fade-in">' + renderWelcome() + "</div>";
    updateBreadcrumb("welcome");
    highlightNav("welcome");
    bindContentEvents();
  }

  function parseAndRenderSlides(container, html) {
    var slideParts;
    if (typeof MarkdownLoader !== "undefined" && MarkdownLoader.renderSlides) {
      slideParts = MarkdownLoader.renderSlides(html);
    } else {
      // Static mode: split by h2 section-title (like Forensik)
      var parts = html.split(/(<h2\s+class="section-title"[^>]*>)/i);
      slideParts = [];
      var firstChunk = parts[0];
      if (firstChunk && firstChunk.trim()) slideParts.push(firstChunk);
      for (var pi = 1; pi < parts.length; pi += 2) {
        var h2tag = parts[pi];
        var content = parts[pi + 1] || "";
        var sectionContent = content.split(/<div\s+class="nav-buttons/i)[0];
        slideParts.push(h2tag + sectionContent);
      }
      if (slideParts.length === 0) slideParts = [html];
    }
    slides = slideParts;
