...
function performSearch(query) {
  ...
  navItems.forEach(function(item) {
    ...
    var content = (labId && typeof ContentData !== "undefined" && ContentData[labId]) ? ContentData[labId][chapterId] : null;
    if (!content) return;
...
function generateTOC() {
  ...
  navItems.forEach(function(item) {
    var labId = (typeof App !== "undefined" && App.currentLab) ? App.currentLab : null;
    var content = (labId && typeof ContentData !== "undefined" && ContentData[labId]) ? ContentData[labId][item.id] : null;
    if (!content) return;
...
function navigateToSearchResult(type, id) {
  if (type === 'chapter' || type === 'section') {
    var labId = (typeof App !== "undefined" && App.currentLab) ? App.currentLab : null;
    var chapterId = id;
    if (labId && typeof ContentData !== "undefined" && ContentData[labId]) {
      if (!ContentData[labId][id]) {
        var parts = id.split('-');
        chapterId = parts.length > 1 ? parts[0] + '-' + parts.slice(1).join('-') : id;
      }
    }
    if (window.App && App.navigateTo) {
