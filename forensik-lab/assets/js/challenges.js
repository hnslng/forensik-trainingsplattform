...
function _getChallengeKey() {
  var labId = (typeof App !== "undefined" && App.currentLab) ? App.currentLab : "linux-forensik";
  return "challenge-completed-" + labId;
}

function _getAttemptKey(challengeId) {
  var labId = (typeof App !== "undefined" && App.currentLab) ? App.currentLab : "linux-forensik";
  return "lab-attempts-" + labId + "-" + challengeId;
}

// ========== ATTEMPT TRACKING ==========
const LabAttempts = {
  getAttempts: function(challengeId) {
    const attempts = localStorage.getItem(_getAttemptKey(challengeId));
    return attempts ? parseInt(attempts) : 0;
  },
  
  incrementAttempts: function(challengeId) {
    const attempts = this.getAttempts(challengeId);
    localStorage.setItem(_getAttemptKey(challengeId), attempts + 1);
    return attempts + 1;
  },
  
  resetAttempts: function(challengeId) {
    localStorage.removeItem(_getAttemptKey(challengeId));
  },
  
  resetAllAttempts: function() {
    var labId = (typeof App !== "undefined" && App.currentLab) ? App.currentLab : "linux-forensik";
    var prefix = "lab-attempts-" + labId + "-";
...
function markChallengeCompleted(challengeId) {
  const completed = JSON.parse(localStorage.getItem(_getChallengeKey()) || '{}');
  completed[challengeId] = true;
  localStorage.setItem(_getChallengeKey(), JSON.stringify(completed));
